...
int copy_page_multithread(struct page *to, struct page *from, int nr_pages)
{
	...
	if (total_mt_num > 32 || total_mt_num < 1)
		return -ENODEV;
	/* pr_info("use %d threads to copy pfn %lx->%lx with %d pages\n", total_mt_num, page_to_pfn(from), page_to_pfn(to), nr_pages); */
...
int copy_page_lists_mt(struct folio **to, struct folio **from, int nr_items)
{
	...
	if (nr_items < total_mt_num) {
		...
		for (item_idx = 0; item_idx < nr_items; ++item_idx) {
			unsigned long chunk_size = PAGE_SIZE * folio_nr_pages(from[item_idx]) / total_mt_num;
			char *vfrom = kmap(&from[item_idx]->page);
			char *vto = kmap(&to[item_idx]->page);
			VM_WARN_ON(PAGE_SIZE * folio_nr_pages(from[item_idx]) % total_mt_num);
			VM_WARN_ON(folio_nr_pages(to[item_idx]) !=
				   folio_nr_pages(from[item_idx]));
		...
		for (cpu = 0; cpu < total_mt_num; ++cpu) {
			...
			for (per_cpu_item_idx = 0; per_cpu_item_idx < work_items[cpu]->num_items;
				...
				work_items[cpu]->item_list[per_cpu_item_idx].to = kmap(&to[item_idx]->page);
				work_items[cpu]->item_list[per_cpu_item_idx].from =
					kmap(&from[item_idx]->page);
				work_items[cpu]->item_list[per_cpu_item_idx].chunk_size =
					PAGE_SIZE * folio_nr_pages(from[item_idx]);
