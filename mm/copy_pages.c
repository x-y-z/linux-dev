		...
		struct list_head *src_folios, int nr_items)
{
	...
	const struct cpumask *per_node_cpumask = cpumask_of_node(push_0_pull_1?to_node:from_node);
	...
	if (nr_items < total_mt_num) {
		...
		list_for_each_entry_safe(src, src2, src_folios, lru) {
			...
			char *vfrom = kmap(&src->page);
			char *vto = kmap(&dst->page);
